...
if [ $# -ne 2]; then
    echo "Please input: 0 <region> <servicename>"
    exit 1
fi 
...
case $aws_service in
    ec2)
        ...
        aws ec2 describe-instances --region $aws_region --query "Reservations[].Instances[].[Tags[?Key=='Name']|[0].Value, State.Name, VpcId]" --output table
        ;;
    rds)
